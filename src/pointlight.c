...
void point_light_begin(point_light_t *l)
{
  ...
  mat4x4_perspective(point_shadow_projection, rad(90.0f), aspect, 0.1f, l->intensity);
...
void point_light_draw(point_light_t *l, GLuint shader)
{
  ...
  glUniform1f(glGetUniformLocation(shader, "u_point_light.intensity"), l->intensity);
}
